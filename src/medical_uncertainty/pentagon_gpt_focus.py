...
def load_gpt_data():
    """Load GPT-5.1 predictions only"""
    checkpoint_path = Path(__file__).parent.parent.parent / "data_dir" / "checkpoints" / "gpt-5.1" / "checkpoint.json"
...
def create_pentagon_gpt_focus():
    ...
    # For each condition, calculate GPT agreement at each vote level
    for cond_idx, (condition, condition_key) in enumerate(zip(conditions, condition_keys)):
        ...
        for vote_level in range(6):  # 0/5 to 5/5
            ...
            if len(patients_at_level) > 0:
                # Calculate GPT agreement for patients at this vote level
                agree_count = 0
                total_count = 0
                
                for patient in patients_at_level:
                    if patient in gpt_df['patient'].values:
                        # Human majority at this level
                        human_vote = vote_level >= 3  # 3+ votes = positive majority
                        # GPT prediction
                        gpt_vote = gpt_df[gpt_df['patient'] == patient][condition_key].iloc[0]
                        
                        total_count += 1
                        if human_vote == gpt_vote:
                            agree_count += 1
    ...
    ax.set_title('GPT-5.1 Agreement with Human Experts by Vote Level\nFull color = high (>80%) | Light color = medium (60-80%) | Gray = low (<60%)',
                fontsize=14, fontweight='bold', pad=30)
    
    return fig

def main():
    print("Creating GPT-focused pentagon visualization...")
    fig = create_pentagon_gpt_focus()
    
    output_dir = Path(__file__).parent.parent.parent / "visualizations"
    output_dir.mkdir(exist_ok=True)
    fig.savefig(output_dir / "pentagon_gpt_focus.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
    
    print("✓ GPT-focused pentagon visualization saved")
