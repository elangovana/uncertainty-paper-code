...
def load_gemini30_data():
    """Load Gemini-3.0-preview predictions only"""
    checkpoint_path = Path(__file__).parent.parent.parent / "data_dir" / "checkpoints" / "gemini30preview" / "checkpoint.json"
...
def create_pentagon_gemini30_focus():
    df = load_groundtruth_data()
    gemini_df = load_gemini30_data()
    ...
    ax.set_title('Gemini-3.0-preview Agreement with Human Experts by Vote Level\nFull color = high (>80%) | Light color = medium (60-80%) | Gray = low (<60%)',
                fontsize=14, fontweight='bold', pad=30)
    
    return fig

def main():
    print("Creating Gemini-3.0-preview focused pentagon visualization...")
    fig = create_pentagon_gemini30_focus()
    
    output_dir = Path(__file__).parent.parent.parent / "visualizations"
    output_dir.mkdir(exist_ok=True)
    fig.savefig(output_dir / "pentagon_gemini30_focus.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
    
    print("✓ Gemini-3.0-preview focused pentagon visualization saved")
